# {PROJECT_NAME}

> **Last Updated:** 2026-01-17 21:41 UTC
\n**Created:** {DATE}
**Template:** Video Editing Template v1.0.0
**Author:** {AUTHOR}
**Project Path:** {PROJECT_PATH}

## Project Structure

- `media/` - General media assets
- `source/` - Original source footage
- `proxies/` - Proxy/optimized media for editing
- `renders/` - Rendered output files
- `project-files/` - Editor project files (Premiere, DaVinci, etc.)
- `exports/` - Final exported deliverables
- `cache/` - Temporary cache and preview files

## Description

{DESCRIPTION}

## Required Plugins

- FFmpeg - Video processing and transcoding
- MKVToolNix - Matroska container manipulation

## Workflow

1. Import source footage to `source/`
2. Generate proxies in `proxies/` for smooth editing
3. Create edits and save projects to `project-files/`
4. Render intermediate files to `renders/`
5. Export final deliverables to `exports/`

## Version Control

Git Enabled: {GIT_ENABLED}

---

<!-- markdownlint-disable MD036 -->
*Managed by pyMediaManager - {DATETIME}*
<!-- markdownlint-enable MD036 -->
