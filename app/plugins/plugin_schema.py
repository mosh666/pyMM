...
class PluginManifestSchema(BaseModel):
    ...
    source: PluginSource = Field(..., description="Download source configuration")
    command: PluginCommand = Field(..., description="Executable command configuration")
    register_to_path: bool = Field(..., description="Whether to add executable directory to PATH")
