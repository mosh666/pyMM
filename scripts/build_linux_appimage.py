...
def get_app_version() -> str:
    """Get application version from setuptools_scm or fallback."""
    try:
        ...
        logger.warning(f"Could not determine version via setuptools_scm: {e}")
        logger.warning("Using fallback version: 0.0.0-dev")
        return "0.0.0-dev"
...
def create_appimage(appdir: Path, version: str, arch: str) -> Path:
    ...
    logger.info(f"✅ AppImage created: {output_path}")
    return output_path
...
def create_checksum(appimage_path: Path) -> Path:
    ...
    logger.info(f"✅ Checksum created: {checksum_path}")
...
def build(python_version: str, arch: str) -> None:
    """Main build function."""
    logger.info(f"🐧 Building Linux AppImage for Python {python_version} ({arch})")
    ...
    logger.info(f"✅ Build complete for Python {python_version} ({arch})")
