...
- **types**: Improve type annotations with future annotations
  ([`8ed0768`](https://github.com/mosh666/pyMM/commit/8ed07685adcfeac026928582cd11214da729c9b2))

Add PEP 563 future annotations and TYPE_CHECKING blocks for better type safety and compatibility:

Changes: - Add 'from __future__ import annotations' to enable PEP 563 - Move type-only imports to
  TYPE_CHECKING blocks - Use modern 'X | None' union syntax instead of Optional[X] - Add noqa
  comment for StorageService false positive (TC001)

Benefits: - Compatible with both Sphinx autodoc and Ruff linting - Reduces runtime import overhead
  for type-only imports - Prevents circular import issues - Satisfies Ruff UP045 (modern union
  syntax) and TC001 rules

Files modified: - app/ui/views/plugin_view.py - app/ui/views/project_view.py -
